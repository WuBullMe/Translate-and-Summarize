...
summarize = Summarizer(src_lan='en')
translate = Translator(src_tg_lan='en_ru')


summarized_news = summarize(news)
translated_news_ru = translate(summarized_news)

print(translated_news_ru)
# output: «Я не планирую быть одним из тех людей, которые, как только им исполнится 18,
# внезапно покупают себе огромную коллекцию спортивных автомобилей, — говорит Гарри Поттер звезда.
# — Я просто думаю, что буду более честной игрой», — говорит он. Лондонец снял телефильм
# под названием «Мой парень и его сын», который должен быть выпущен позднее в этом году.